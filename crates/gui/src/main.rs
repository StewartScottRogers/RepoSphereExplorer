...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Held outside `windows` so `.run()` below never has to borrow it: the
    // event loop it starts is what pop-out and dock (#617) themselves run
    // inside of, and a borrow held across it would make the first of
    // either panic.
    let run_handle = ui
        .as_weak()
        .upgrade()
    ...
    let tick_windows = windows.clone();
    ...
    gui::settings::save_pane_widths(gui::settings::PaneWidths {
        folders: run_handle.get_folders_width(),
        contents: run_handle.get_contents_width(),
    });
    if let Some(geometry) = gui::geometry_to_save(&run_handle, &geometry_tracker) {
