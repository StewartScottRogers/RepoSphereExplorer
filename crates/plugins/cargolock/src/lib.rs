...
/// The source a package resolved from crates.io records, which is in
/// every lock file that has a dependency at all.
...
fn looks_like_it(text: &str) -> bool {
    ...
    // Two `[[package]]` tables is not enough on its own: a `uv.lock`
    // and a `poetry.lock` are TOML with the same tables, and this
    // plugin has never been able to read either. Cargo writes the
    // generated banner on every lock file it creates, and a resolution
    // that came from the registry says where it came from.
    generated || (packages >= 2 && text.contains(CRATES_IO))
}
...
mod tests {
    ...
    #[test]
    fn sniffs_the_generated_banner_or_a_registry_resolution() {
        assert!(
            CargolockCore
                .sniff(b"# This file is automatically @generated by Cargo.\nversion = 4\n")
        );
        assert!(CargolockCore.sniff(
            b"[[package]]\nname = \"a\"\n\
    ...
    #[test]
    fn a_python_lock_file_is_not_a_cargo_one() {
        // Two `[[package]]` tables used to be enough, and `uv.lock` and
        // `poetry.lock` are TOML with exactly those. This plugin has
        // never been able to read either.
        assert!(!CargolockCore.sniff(
            b"version = 1\nrequires-python = \">=3.9\"\n\n\
              [[package]]\nname = \"click\"\nversion = \"8.1.7\"\n\n\
              [[package]]\nname = \"rich\"\nversion = \"13.7.1\"\n"
        ...
        assert!(!CargolockCore.sniff(
            b"# This file is automatically @generated by Poetry 1.8.3.\n\n\
              [[package]]\nname = \"click\"\n\n[[package]]\nname = \"rich\"\n"
        ));
    }
