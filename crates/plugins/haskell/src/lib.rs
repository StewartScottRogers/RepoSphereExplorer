...
fn has_haskell_syntax(text: &str) -> bool {
    // Any pragma, not only `LANGUAGE`. A test entry point generated by
    // hspec is one line - `{-# OPTIONS_GHC -F -pgmF hspec-discover #-}`
    // - and `{-#` opens a pragma in Haskell and in nothing else.
    text.contains("{-#")
...
mod tests {
    ...
    #[test]
    fn sniffs_a_one_line_pragma_file_as_haskell() {
        // What hspec generates as `test/Spec.hs`, and all it contains.
