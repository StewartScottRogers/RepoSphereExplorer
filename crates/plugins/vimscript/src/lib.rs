...
/// told.
const VIMSCRIPT: Language = Language {
    line_comment: &["\""],
    block_comment: &[],
    quotes: &[Quote::simple('"')],
    keywords: &[
...
impl PluginPresentation for VimscriptPresentation {
    fn classify(&self, text: &str) -> Vec<Span> {
        syntax::classify(text, &VIMSCRIPT)
    }
...
mod tests {
    ...
    fn both_halves_claim_the_same_extensions() {
        assert_eq!(
            plugin_api::PluginCore::extensions(&crate::VimscriptCore),
            plugin_api::PluginPresentation::extensions(&crate::VimscriptPresentation),
