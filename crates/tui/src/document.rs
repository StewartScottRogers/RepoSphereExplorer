...
//! over it.
//!
//! Nothing here draws or reads a key. That split is deliberate, and mirrors
//! `crates/gui/src/document.rs`, which this is a front-end-neutral copy of:
//! a rule that lives in pure Rust can be exercised without an event loop,
//! and a caret that goes to the wrong column on the third line of a file is
//! not something a person notices by looking once.
//!
...
mod tests {
    ...
    fn typing_replaces_what_is_selected() {
        let mut document = at("|hello world");
        document.move_right(true);
        document.move_right(true);
        document.move_right(true);
        document.move_right(true);
