...
| SQL | `crates/plugins/sql` | #24 | Sniffs by statement keywords at the start of a line (`SELECT`, `INSERT INTO`, `UPDATE`, `DELETE FROM`, `CREATE TABLE`/`INDEX`/`VIEW`, `ALTER TABLE`, `DROP TABLE`) or a `PRIMARY KEY`/`FOREIGN KEY` constraint anywhere, none of which are used by any sibling plugin; placed just ahead of `text` in `CORE_PLUGINS`, no ordering constraint against a specific sibling since it has no overlapping markers |
