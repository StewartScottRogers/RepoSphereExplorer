...
| Lua source | `crates/plugins/lua` | #21 | Sniffs by a `lua` shebang or Lua-only markers (a trimmed line starting with `local `, a `--` comment prefix, `elseif `, or a line ending in ` then`) not used by this project's other source-language plugins; deliberately does not sniff on `require(`, since a Lua `require("module")` call would collide with the JavaScript plugin's bare `require(` substring check; placed just ahead of `text` in `CORE_PLUGINS` |
