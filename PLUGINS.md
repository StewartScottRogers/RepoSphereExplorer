...
| Lua source | `crates/plugins/lua` | #21 | Sniffs by a `lua` shebang, a `local ` declaration, a `--`/`--[[` comment opener, or an `elseif ` keyword — markers not used by any sibling plugin; deliberately does not sniff a bare `end` line, since the Ruby plugin already claims that marker, nor a `then`-ending line, since the shell plugin already claims a `; then` ending that a bare `then` check would still overlap; placed just ahead of `text` in `CORE_PLUGINS`, no ordering constraint against a specific sibling since it has no overlapping markers |
